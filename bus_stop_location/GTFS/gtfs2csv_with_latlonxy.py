...
def get_stops_data(): #Converts the XML-Dtaa in the additional file into a Pandas DataFrame, checks for faulty Stops
    path_py = pathlib.Path(__file__)
    path_stops = path_py.with_name("gtfs_with_dhid_copy.add.xml")
...
for index in range(0, len(df_busStops)):
    x,y = sumolib.geomhelper.positionAtShapeOffset(net.getLane(df_busStops._get_value(index, 'lane')).getShape(), 0)
...
df_csv.to_csv('gtfs_stations_copy.csv', index=False)
